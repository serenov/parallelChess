...
static inline Bitboard getPseudoLegalMoves(Boards* boards, Square square) {
    ...
    case 'Q':
        return 
        __attackRank__[square][getRankState(boards, square)] | 
    ...
    case 'N':
        return 
        __knightAttack__[square];boards -> state.colorToPlay;

    case 'P':
        return 
        __pawnAttack__[!(boards -> state.colorToPlay)][square];
...
static inline Bitboard castleRightMove(Boards* boards, Color color, Bitboard unsafeSquares){
    ...
    if(boards -> state.canCastleRight[color]) return (color == white)? 3UL << 5: 3UL << 61;
...
static inline Bitboard castleLeftMove(Boards* boards, Color color, Bitboard unsafeSquares) {
    ...
    if(boards -> state.canCastleLeft[color]) return (color == white)? 3UL << 3: 3UL << 59;
